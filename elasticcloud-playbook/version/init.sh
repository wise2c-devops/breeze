...
curl -L -o ${path}/template/operator.yml.j2 https://download.elastic.co/downloads/eck/${ElasticCloudVersion}/operator.yaml

cat ${path}/template/operator.yml.j2 |grep 'image: "docker.elastic.co/eck/' |awk -F":" '{print $2":"$3}' |awk -F'"' '{print $2}' > images-list.txt
...
sed -i "s,docker.elastic.co/eck,{{ registry_endpoint }}/{{ registry_project }},g" ${path}/template/operator.yml.j2
